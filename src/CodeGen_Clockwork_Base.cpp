...
string CodeGen_Clockwork_Base::print_stencil_type(HW_Stencil_Type stencil_type) {
    ostringstream oss;
    // C: Stencil<uint16_t, 1, 1, 1> stencil_var;
    // C: hls::stream<Stencil<uint16_t, 1, 1, 1> > stencil_stream_var;

    switch(stencil_type.type) {
    case HW_Stencil_Type::StencilContainerType::Stencil :
        ...
        oss << ">";
        break;
    case HW_Stencil_Type::StencilContainerType::Stream :
        ...
        oss << "> >";
        break;
    case HW_Stencil_Type::StencilContainerType::AxiStream :
...
void CodeGen_Clockwork_Base::visit(const Call *op) {
    if (op->name == "linebuffer") {
        ...
        if (op->args.size() == 2) {
            ...
            HW_Stencil_Type stencil_type = stencils.get(stencil_name);
            internal_assert(stencil_type.type == HW_Stencil_Type::StencilContainerType::Stencil);
        ...
        HW_Stencil_Type stream_type = stencils.get(stream_name);
        ...
        for (size_t i = 0; i < num_of_consumers; i++) {
            ...
            HW_Stencil_Type consumer_stream_type = stream_type;
        ...
        HW_Stencil_Type stencil_type = stream_type;
        stencil_type.type = HW_Stencil_Type::StencilContainerType::Stencil;
...
void CodeGen_Clockwork_Base::visit(const Realize *op) {
    if (ends_with(op->name, ".stream")) {
        ...
        HW_Stencil_Type stream_type({HW_Stencil_Type::StencilContainerType::Stream,
