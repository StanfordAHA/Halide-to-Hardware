...
namespace {
const string vhls_header_includes =
    "#include <assert.h>\n"
    "#include <stdio.h>\n"
    "#include <stdlib.h>\n"
...
void CodeGen_VHLS_Target::init_module() {
    debug(1) << "CodeGen_VHLS_Target::init_module\n";
    ...
    string module_name = "HALIDE_CODEGEN_VHLS_TARGET_" + target_name + "_H";
    ...
    hdr_stream << vhls_header_includes << '\n';
...
void CodeGen_VHLS_Target::add_kernel(Stmt s,
                                    const string &name,
                                    const vector<VHLS_Argument> &args) {
    debug(1) << "CodeGen_VHLS_Target::add_kernel " << name << "\n";

    hdrc.add_kernel(s, name, args);
    srcc.add_kernel(s, name, args);
}

void CodeGen_VHLS_Target::dump() {
    std::cerr << src_stream.str() << std::endl;
}


string CodeGen_VHLS_Target::CodeGen_VHLS_C::print_stencil_pragma(const string &name) {
    ostringstream oss;
...
void CodeGen_VHLS_Target::CodeGen_VHLS_C::add_kernel(Stmt stmt,
                                                   const string &name,
                                                   const vector<VHLS_Argument> &args) {
    ...
    for (size_t i = 0; i < args.size(); i++) {
        ...
        if (args[i].is_stencil) {
            CodeGen_VHLS_Base::Stencil_Type stype = args[i].stencil_type;
...
void CodeGen_VHLS_Target::CodeGen_VHLS_C::visit(const For *op) {
...
void CodeGen_VHLS_Target::CodeGen_VHLS_C::visit(const Allocate *op) {
