...
string CodeGen_VHLS_Base::print_stencil_pragma(const string &name) {
...
void CodeGen_VHLS_Base::visit(const Call *op) {
  if (op->name == "linebuffer") {
        ...
        for (int i = num_of_demensions - 1; i >= 0; i--) {
            ...
            do_indent();
            // VHLS C: for(int dim = 0; dim <= store_extent - stencil.size; dim += stencil.step)
...
void CodeGen_VHLS_Base::visit(const Realize *op) {
